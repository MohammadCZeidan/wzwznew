...
import { REQUEST_PACKAGES } from "@/lib/tokenRequests";

export function DashboardWallet() {
  const { tokenBalance: balance } = useRawStore();
  const [requestOpen, setRequestOpen] = useState(false);
  const [requestPrice, setRequestPrice] = useState<number | undefined>(undefined);

  const openRequest = (price?: number) => {
    setRequestPrice(price && REQUEST_PACKAGES.includes(price as never) ? price : undefined);
    setRequestOpen(true);
  };
    ...
    <div className="space-y-6 sm:space-y-8">
      <header className="space-y-1">
        ...
        <p className="text-xs text-raw-silver/45 sm:text-sm">
          Request tokens to unlock features, boost rewards, and access premium content.
        </p>
      </header>
          ...
          <div className="relative px-4 pb-4 pt-4 sm:px-8 sm:pb-7 sm:pt-8">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-5">
              ...
              <div className="flex flex-col gap-1.5 sm:items-end">
                <button
                  onClick={() => openRequest(5)}
                  className="w-full rounded-xl border border-raw-gold/40 bg-raw-gold/15 px-6 py-3 text-sm font-semibold text-raw-gold transition hover:bg-raw-gold/25 sm:w-auto sm:rounded-2xl sm:px-8 sm:py-3.5"
                >
                  Request Tokens
                </button>
                <p className="text-center text-[10px] text-raw-silver/25 sm:text-right">Paid plans aren't live yet</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <p className="text-xs text-raw-silver/35">
        Paid token purchases aren't live yet — request a package and we'll get you topped up. Tokens never expire.
      </p>

      <RequestTokensModal
        isOpen={requestOpen}
        onClose={() => setRequestOpen(false)}
        defaultPriceUsd={requestPrice}
      />
    </div>
  );
}
